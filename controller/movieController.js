...
exports.addMovie = async (req, res, next) => {
  ...
  exports.listDirector = async (req, res, next) => {
    try {
      ...
      if (error.response) {
        console.error('Server Error:', error.response.data);
        res.status(error.response.status).render("../views/movie/directors/list_directors.ejs", {
          error: 'Đã xảy ra lỗi khi lấy danh sách đạo diễn.'
        });
      } else if (error.request) {
        console.error('Request Error:', error.request);
        res.status(500).render("../views/movie/directors/list_directors.ejs", {
          error: 'Không nhận được phản hồi từ server khi lấy danh sách đạo diễn.'
        });
      } else {
        console.error('Error:', error.message);
        res.status(500).render("../views/movie/directors/list_directors.ejs", {
          error: 'Đã xảy ra lỗi khi xử lý yêu cầu lấy danh sách đạo diễn.'
        });
      }
    }
  };
